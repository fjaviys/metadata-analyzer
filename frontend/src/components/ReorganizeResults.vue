    ...
    <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-3">
        ...
        <button v-if="canUndo" class="btn-ghost text-red-600" :disabled="undoing" @click="undo">
          {{ undoing ? 'Deshaciendo…' : 'Deshacer este run' }}
        </button>
      </div>
    </div>
      ...
      <table class="w-full text-left text-sm">
        <thead class="border-b border-slate-200 text-xs uppercase text-slate-500">
          <tr>
            ...
            <th class="py-2 pr-3">Motivo / error</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="r in rows" :key="r.id" class="border-b border-slate-100">
            <td class="max-w-xs truncate py-2 pr-3 text-slate-700" :title="r.original_path">
              {{ short(r.original_path) }}
            </td>
            ...
            <td class="max-w-xs truncate py-2 pr-3 text-xs text-slate-500"
...
async function reload() {
  loading.value = true;
  try {
    const r = await api.getReorganizeRun(props.runId, {
...
function exportCsv() {
  const header = ['origen', 'destino', 'estado', 'motivo_error'];
  const lines = rows.value.map((r) => [
    r.original_path, r.new_path ?? '', r.status, r.error || r.reason || '',
  ...
  a.href = url;
  a.download = `reorganizacion_${props.runId}.csv`;
  a.click();
